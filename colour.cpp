...
#pragma HLS INTERFACE m_axi depth=2073600 port=out_data offset=slave
	...
	if(case_t < 10){//TEST COLOUR
	  ...
	  //unsigned char  r_avg = 180,g_avg=120,b_avg=120;
		unsigned char  r_low = r_avg - error;
		unsigned char  r_high = r_avg + error;
		unsigned char  g_low = g_avg - error;
		unsigned char  g_high = g_avg + error;
		unsigned char  b_low = b_avg - error;
		unsigned char  b_high = b_avg + error;
		int max_hist = 0;
		...
		if(y_bird_tmp > s_bird and y_bird < h-s_bird){//floor ceiling
		...
		for (int i = 0; i < h; ++i) {
			...
			for (int j = 0; j < w; ++j) {
				...
				if(middle <= 8 and case_t>20){
					if(y_position[middle] > y_bird or y_position[middle]+gap < y_bird+s_bird){
						game_over = true;
					}
				}
				...
				for (s8b i=0; i < COUNT; i++){//smallest positive
				...
				while(!first_pillar and pillar_index < COUNT){
