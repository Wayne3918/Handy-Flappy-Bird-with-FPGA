...
void template_filter(volatile uint32_t* in_data, volatile uint32_t* out_data, word12 w, word12 h, word& x_width, package& x_pillar, word interval,  word v_horizontal, word& s_bird,  word& y_bird, package& y_depth, word& new_depth,word gap, bool& game_over, unsigned char& r_avg, unsigned char& g_avg, unsigned char& b_avg, unsigned char& case_t, unsigned char error, word12& max_h,word12 leftbound, word12 rightbound, word in_g_max, word in_b_max, int& fail);
...
for(int i = 0; i < 9; i++){
	...
	template_filter((volatile uint32_t*) image, (volatile uint32_t*) image,  1280,  720, x_width, x_pillar, interval,  v_horizontal, s_bird, y_bird, y_depth, new_depth, gap, game_over , r_avg, g_avg, b_avg,  case_t, error, max_h,game_over,100,220,200,fail);
