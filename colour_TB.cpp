...
void encode_x(package& x_pillar, s12b x_position[COUNT]){
	x_pillar = 0;
...
void template_filter(volatile uint32_t* in_data, volatile uint32_t* out_data, int& w, int& h, s12b& x_width, package& x_pillar, s12b interval,  s12b v_horizontal, s12b& s_bird,  s12b& y_bird, package& y_depth, s12b& new_depth,s12b gap, bool game_over, unsigned char& r_avg, unsigned char& g_avg, unsigned char& b_avg, unsigned char  case_t, unsigned char error);
...
for(int i = 0; i < 9; i++){
	...
	template_filter((volatile uint32_t*) image, (volatile uint32_t*) image,  w,  h, x_width, x_pillar, interval,  v_horizontal, s_bird, y_bird, y_depth, new_depth, gap, game_over , r_avg, g_avg, b_avg,  case_t, error);
